...
export const Footer: React.FC<FooterProps> = ({ site, socials = SOCIALS }) => {
  const goBackToTop = (event: React.MouseEvent<HTMLButtonElement>) => {
              ...
              <div className="order-1 sm:order-2 flex justify-center sm:justify-end">
                <div className="flex flex-wrap gap-1 items-center justify-center">
                  {socials.map((social) => (
                    ...
                    >
                      <social.ICON className="size-full fill-current group-hover:fill-black group-hover:dark:fill-white blend" />
                    </a>
                  ))}
                </div>
              </div>
